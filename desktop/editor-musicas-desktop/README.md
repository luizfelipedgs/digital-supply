...
Versão desktop (Windows, `.exe`) do Editor de Músicas. Mesma ideia da versão web
(`/dashboard/editor-musicas`): escolhe uma música, escolhe vários vídeos, e o programa
...
```bash
cd desktop/editor-musicas-desktop
npm install
...
instalador automaticamente sempre que algo muda em `desktop/editor-musicas-desktop`
...
**Actions > Build desktop app (Editor de Músicas) > Run workflow**) builda o
`.exe` e:
